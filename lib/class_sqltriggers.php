    ...
    protected static function _player($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            ...
            case T_SQLTRIG_PLAYER_DPROPS: 
                $conn->query("CALL getPlayerDProps($pid, @inj_ma,@inj_av,@inj_ag,@inj_st,@inj_ni, @ma,@av,@ag,@st, @ma_ua,@av_ua,@ag_ua,@st_ua, @value, @status, @date_died)") or die($conn->errorInfo());
                $conn->query("UPDATE players SET
                            ...
                            value = @value, status = @status, date_died = @date_died WHERE player_id = $pid") or die($conn->errorInfo());
            ...
            case T_SQLTRIG_PLAYER_RELS: 
                $conn->query("CALL getPlayerRels($pid, @f_cid, @f_rid, @f_cname, @f_rname, @f_tname, @f_pos_name)") or die($conn->errorInfo());
                $conn->query("UPDATE players SET f_cid = @f_cid, f_rid = @f_rid, f_cname = @f_cname, f_rname = @f_rname, f_tname = @f_tname, f_pos_name = @f_pos_name WHERE player_id = $pid") or die($conn->errorInfo());
                break;
        }
    }
    ...
    protected static function _team($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            ...
            case T_SQLTRIG_TEAM_DPROPS: 
                $conn->query("CALL getTeamDProps($tid, @tv, @ff)") or die($conn->errorInfo());
                $conn->query("UPDATE teams SET tv = @tv, ff = @ff WHERE team_id = $tid") or die($conn->errorInfo());
                break;
            
            case T_SQLTRIG_TEAM_RELS: 
                $conn->query("CALL getTeamRels($tid, @f_cname, @f_rname)") or die($conn->errorInfo());
                $conn->query("UPDATE teams SET f_cname = @f_cname, f_rname = @f_rname WHERE team_id = $tid") or die($conn->errorInfo());
                break;
                
            case T_SQLTRIG_TEAM_UPDATE_CHILD_RELS: 
                $conn->query("UPDATE players,teams SET f_tname = teams.name, f_cid = owned_by_coach_id, players.f_cname = teams.f_cname WHERE team_id = $tid AND owned_by_team_id = team_id") or die($conn->errorInfo());
                $conn->query("UPDATE mv_players,teams SET f_cid = owned_by_coach_id WHERE team_id = $tid AND f_tid = team_id") or die($conn->errorInfo());
                $conn->query("UPDATE mv_teams,teams SET f_cid = owned_by_coach_id WHERE team_id = $tid AND f_tid = team_id") or die($conn->errorInfo());
                self::_coach(T_SQLTRIG_COACH_TEAMCNT, array('id' => $t->owned_by_coach_id, 'obj' => new Coach($t->owned_by_coach_id)));
                break;
        }
    }
    ...
    protected static function _coach($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            case T_SQLTRIG_COACH_UPDATE_CHILD_RELS: 
                $conn->query("UPDATE players,coaches SET f_cname = coaches.name WHERE coach_id = $cid AND f_cid = coach_id") or die($conn->errorInfo());
                $conn->query("UPDATE teams,coaches SET f_cname = coaches.name WHERE coach_id = $cid AND owned_by_coach_id = coach_id") or die($conn->errorInfo());
                break;
                
            case T_SQLTRIG_COACH_TEAMCNT:
                $conn->query("UPDATE coaches SET team_cnt = getTeamCnt(".T_OBJ_COACH.", $cid, NULL) WHERE coach_id = $cid") or die($conn->errorInfo());
                break;
        }
    }
    ...
    protected static function _race($T_SQLTRIG, array $argv)
    {
        ...
        switch ($T_SQLTRIG)
        {
            case T_SQLTRIG_RACE_TEAMCNT:
                $conn->query("UPDATE races SET team_cnt = getTeamCnt(".T_OBJ_RACE.", $rid, NULL) WHERE race_id = $rid") or die($conn->errorInfo());
                break;
        }
    }
    ...
    protected static function _match($T_SQLTRIG, array $argv)
    {
        switch ($T_SQLTRIG)
        {
            case T_SQLTRIG_MATCH_UPD:
                $conn->query("CALL match_sync($argv[mid], $argv[trid], $argv[tid1], $argv[tid2], $argv[played])") or die($conn->errorInfo());
                break;

            case T_SQLTRIG_MATCH_DEL:
                $conn->query("CALL match_sync($argv[mid], $argv[trid], $argv[tid1], $argv[tid2], TRUE)") or die($conn->errorInfo());
                break;
        }
    }
}
